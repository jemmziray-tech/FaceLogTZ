...
# 3. Build the Main Application Window
class FaceLogApp(ctk.CTk):
    def __init__(self):
        ...
        self.video_label = ctk.CTkLabel(self.main_frame, text="Loading Database...", font=ctk.CTkFont(size=20))
        self.video_label.pack(pady=20)

        self.start_button = ctk.CTkButton(self.main_frame, text="Start Camera Stream", font=ctk.CTkFont(size=16, weight="bold"), height=50, command=self.start_camera)
    ...
    def markAttendance(self, name):
        ...
        with open('Attendance.csv', 'r+') as f:
            ...
            nameList = [line.split(',')[0] for line in myDataList if line.strip()]
            ...
            if name not in nameList:
                ...
                # --- TRIPLE THREAT MULTITHREADING ---
                # 1. Talk to the user locally
                threading.Thread(target=speak, args=(name,)).start()
                # 2. Text the parent globally
                threading.Thread(target=send_whatsapp, args=(name, timeString)).start()
    ...
    def start_camera(self):
        if self.cap is None:
            ...
            if not self.cap.isOpened():
                self.video_label.configure(text="ERROR: Camera is blocked by another app!", text_color="red")
                self.cap = None
                return
